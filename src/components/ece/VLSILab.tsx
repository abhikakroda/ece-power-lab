...
const VLSILab = () => {
  const [activeTab, setActiveTab] = useState<VLSITab>("mosfet");

  return (
    <div className="p-6 md:p-10 max-w-6xl mx-auto space-y-6">
      <div className="flex items-center gap-3">
        <div className="w-10 h-10 rounded-lg bg-chart-4/20 flex items-center justify-center">
          <Cpu size={20} className="text-chart-4" />
        </div>
        <div>
          <h2 className="text-2xl font-bold text-foreground">VLSI Technology Lab</h2>
          <p className="text-sm text-muted-foreground font-mono">MOSFET Physics • Fabrication Flow • Device Scaling</p>
        </div>
      </div>
