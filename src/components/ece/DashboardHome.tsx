  ...
  return (
    <div className="p-6 md:p-10 max-w-5xl mx-auto space-y-10">
      ...
      <div className="pt-8 pb-4">
        <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full border border-primary/20 bg-primary/5 text-primary text-sm font-medium mb-5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {features.map((f, i) => (
          <button
            ...
            className="group text-left p-6 rounded-xl border border-border bg-card hover:bg-muted/50
              hover:border-primary/20 transition-all duration-200 animate-fade-in"
            style={{ animationDelay: `${i * 60}ms` }}
          >
            <div className="w-10 h-10 rounded-lg bg-primary/8 flex items-center justify-center mb-3 text-primary">
              {f.icon}
            </div>
            <h3 className="text-foreground font-medium text-base mb-1.5">{f.title}</h3>
      ...
      <div className="grid grid-cols-4 gap-4">
        {[
          ...
          <div key={stat.label} className="p-4 rounded-lg bg-card border border-border text-center">
            <div className="text-xl font-semibold font-mono text-foreground">{stat.value}</div>
